...
map = [[-125, -155, 2, 30];
 ...
 [-55, -55, 2, 10];
 [-155, -15, 2, 10];
 [195,195,2,100];[-195,195,2,100];[-195,-195,2,100];[195,-195,2,100]];
tollerance = 2.5;
points = 0;
%% ************************* RUN SIMULATION *************************
...
for iter = 1:max_iter
    ...
    abc(iter,1)=time;
    abc(iter,2:4)=current_state.pos;
    abc(iter,5:7)= current_state.rot;
    abc(iter,8:10)= current_state.vel;
    abc(iter,11:13)= current_state.omega;
    
    desired_state = trajhandle(time + cstep, current_state);
    def(iter,1)=time;
    def(iter,2:4)=desired_state.pos;
    ...
    %Judging if the points visited
    for k = 1: 29
        if((abs(current_state.pos(1)-map(k,1))<tollerance)&& (abs(current_state.pos(2)-map(k,2))<tollerance) && (abs(current_state.pos(3)-map(k,3))<tollerance))
            points = points + map(k,end);
            map(k,:)=[0,0,0,0];
        end
...
disp('finished.')
writematrix(abc,"current.csv");
writematrix(def,"desired.csv");
t_out = ttraj;
s_out = xtraj;
